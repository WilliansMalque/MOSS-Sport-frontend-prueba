...
const CrearPartido = ({ open, onClose, onCreate }) => {
      ...
      <Modal open={open} onClose={onClose}>
        ...
        >
          ...
          <FormControl fullWidth margin="normal">
            ...
            >
              {torneos.map((torneo) => {
                console.log('Torneo key:', torneo.id);
                return (
          ...
          <FormControl fullWidth margin="normal" disabled={!equiposFiltrados.length}>
            ...
            >
              {equiposFiltrados.map((equipo) => {
                console.log('Equipo key:', equipo.equipo_id);
                return (
    ...
    <Select
        ...
        onChange={(e) => {
            const equipoSeleccionado = equiposFiltrados.find(
                (equipo) => equipo.equipo_id === parseInt(e.target.value, 10)
            );
            console.log('[INFO] Equipo Visitante seleccionado:', equipoSeleccionado);

            setNuevoPartido((prev) => ({
                ...prev,
                equipo_visitante_id: equipoSeleccionado?.equipo_id || null,
            }));
        }}
    >
        {equiposFiltrados.map((equipo, index) => (
            <MenuItem key={`${equipo.equipo_id}-${index}`} value={equipo.equipo_id}>
                {equipo.equipo_nombre} - {equipo.categoria_nombre}
            </MenuItem>
        ))}
    </Select>
